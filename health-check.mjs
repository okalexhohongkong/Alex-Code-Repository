...
const batchData = await fetchText("/界面原型-v1/archive-batch-data.js");
assertOk(batchData.response.ok, "浏览器批次摘要无法访问。");
assertOk(batchData.text.includes("HWS_LATEST_SCAN_BATCH"), "浏览器批次摘要缺少批次对象。");
assertOk(!/\/Users\/|\/Volumes\/|[A-Za-z]:\\/.test(batchData.text), "浏览器批次摘要仍包含本机绝对路径。");
checks.push(["浏览器批次摘要", "通过"]);

const batchHistoryData = await fetchText("/界面原型-v1/archive-batch-history-data.js");
assertOk(batchHistoryData.response.ok, "浏览器批次历史无法访问。");
assertOk(batchHistoryData.text.includes("HWS_SCAN_BATCH_HISTORY"), "浏览器批次历史缺少历史对象。");
assertOk(!/\/Users\/|\/Volumes\/|[A-Za-z]:\\/.test(batchHistoryData.text), "浏览器批次历史仍包含本机绝对路径。");
checks.push(["浏览器批次历史", "通过"]);

const mergedIndexData = await fetchText("/界面原型-v1/archive-merged-index-data.js");
assertOk(mergedIndexData.response.ok, "浏览器合并索引无法访问。");
assertOk(mergedIndexData.text.includes("HWS_MERGED_ARCHIVE_INDEX"), "浏览器合并索引缺少合并索引对象。");
assertOk(!/\/Users\/|\/Volumes\/|[A-Za-z]:\\/.test(mergedIndexData.text), "浏览器合并索引仍包含本机绝对路径。");
checks.push(["浏览器合并索引", "通过"]);
